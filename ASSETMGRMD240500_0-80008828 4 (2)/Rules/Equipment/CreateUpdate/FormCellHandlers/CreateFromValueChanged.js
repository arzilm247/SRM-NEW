...
function updateRelatedControls(context, createFromKey) {
    ...
    if (previousKeySelected) {
        return context.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipments', [], '$filter=sap.islocal()&$orderby=EquipId').then(result => {
            if (result.length === 1) {
