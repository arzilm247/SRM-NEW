...
export default function NotificationCreateUpdateOnPageLoad(context) {
    ...
    try {
        // AllowDefaultValueIfOneItem was enabled for DamageGroupLstPkr, CauseGroupLstPkr and PartGroupLstPkr.
        // onValueChange event is not fired by the page due to the SectionedTable page type.
        // we need to manually trigger the onValueChange event for the controls.
        if (context.evaluateTargetPath('#Control:DamageGroupLstPkr').getValue().length) {
            context.evaluateTargetPath('#Control:DamageGroupLstPkr').triggerOnValueChangeEventForOneItemSelected();
        }
        if (context.evaluateTargetPath('#Control:CauseGroupLstPkr').getValue().length) {
            context.evaluateTargetPath('#Control:CauseGroupLstPkr').triggerOnValueChangeEventForOneItemSelected();
        }
        if (context.evaluateTargetPath('#Control:PartGroupLstPkr').getValue().length) {
            context.evaluateTargetPath('#Control:PartGroupLstPkr').triggerOnValueChangeEventForOneItemSelected();
        }
    } catch (error) {
