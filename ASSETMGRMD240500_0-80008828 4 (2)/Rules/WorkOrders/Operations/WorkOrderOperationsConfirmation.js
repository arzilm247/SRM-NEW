...
export default function WorkOrderOperationsConfirmation(context) {
    ...
    return Promise.all(promiseArr).then(() => {
        if (assignmentType === 'Header' && IsBulkConfirmationSignatureFlowActive(context)) {
            const completeAction = getOperationCompleteAction(context, operationsConfirmations[operationsConfirmations.length - 1]);
