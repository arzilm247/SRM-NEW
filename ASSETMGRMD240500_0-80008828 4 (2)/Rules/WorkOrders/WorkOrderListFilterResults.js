...
export function GetWorkOrderListFilterCriteria(context, saveToClientData = false) {
    ...
    const assignedToFilter = GetAssigmentFilter(context, saveToClientData);
...
function GetAssigmentFilter(context, saveToClientData = false) {
    ...
    if (assignments.length > 0) { //Assignee rows for supervisor selected
        ...
        if (title) {
            ...
            return context.createFilterCriteria(context.filterTypeEnum.Filter, undefined, undefined, filter, true);
        }
    }
    return null;
}
