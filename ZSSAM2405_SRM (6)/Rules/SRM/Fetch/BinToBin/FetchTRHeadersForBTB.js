...
export default function FetchTRHeaderOnline(clientAPI) {
    let TransferOrder = clientAPI
        .evaluateTargetPath('#Page:BTBSearch_Transfer_Order/#Control:Document_Number')
        .getValue();
    let warehouseValue = clientAPI
        .evaluateTargetPath('#Page:BTBSearch_Transfer_Order/#Control:FormCellListPicker0')
        .getValue();
    let warehouse = '';
    ...
    let FromDate = clientAPI
        .evaluateTargetPath('#Page:BTBSearch_Transfer_Order/#Control:Doc_date_from')
        .getValue();
    let ToDate = clientAPI
        .evaluateTargetPath('#Page:BTBSearch_Transfer_Order/#Control:Doc_date_to')
        .getValue();
    let Material = clientAPI
        .evaluateTargetPath('#Page:BTBSearch_Transfer_Order/#Control:Material')
        .getValue();
    ...
    return clientAPI.executeAction('/ZSSAM2405_SRM/Actions/SRM/OData/CreateOnlineOData.action')
        ...
        .then(result => {
            if (result && result.length > 0) {
                ...
                return clientAPI.executeAction('/ZSSAM2405_SRM/Actions/SRM/BinToBin/NavTo_BTB_List.action');
            } else {
